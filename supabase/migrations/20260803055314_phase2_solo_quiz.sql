...
## Summary
Creates tables for solo quiz attempts, answers, skill levels, and seen questions.
Adds the submit_quiz_attempt SECURITY DEFINER RPC for authoritative scoring.
Adds update_skill_score RPC.

## New Tables
- quiz_attempts: solo quiz sessions with score, state, and timing
- quiz_answers: per-question detail for review
- user_skill_levels: adaptive skill score per user per topic
- seen_questions: dedup tracking (with pruning capability)

## Security
- All score/skill writes via RPC only — no client INSERT/UPDATE on score columns
- quiz_attempts: user can INSERT (with score=0) and SELECT own; UPDATE only via RPC
- quiz_answers: user can INSERT and SELECT own
- user_skill_levels: user can SELECT own; INSERT/UPDATE only via RPC
- seen_questions: user can SELECT/INSERT own
*/
...
-- submit_quiz_attempt: authoritative scoring RPC
...
BEGIN
  ...
  -- Update skill score: new = clamp(1,100, round(current*0.75 + accuracy*0.25))
...
-- get_skill_score: read a user's skill score for a topic
...
-- prune_seen_questions: cap seen questions per user to prevent unbounded growth
